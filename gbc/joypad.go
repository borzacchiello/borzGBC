...
func (j *Joypad) Load(decoder *gob.Decoder) error {
	errs := []error{
		decoder.Decode(&j.BackState),
		decoder.Decode(&j.FrontState),
		decoder.Decode(&j.ActionSelector),
		decoder.Decode(&j.DirectionSelector),
		decoder.Decode(&j.ticks),
	}
